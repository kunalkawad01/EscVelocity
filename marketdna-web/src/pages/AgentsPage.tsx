        ...
        <Box sx={{ display: 'flex', gap: 6, flexWrap: 'wrap', mb: 5 }}>
          ...
          {NAV.map(col => (
            <Box key={col.label} sx={{ flex: '1 1 140px' }}>
              <Typography sx={{ ...JAKARTA, fontSize: '0.7rem', fontWeight: 700, color: CYAN, letterSpacing: '0.12em', textTransform: 'uppercase', mb: 1.75 }}>{col.label}</Typography>
              {col.links.map(link => (
                <Typography key={link} sx={{ ...JAKARTA, fontSize: '0.8rem', color: INK3, mb: 0.875, cursor: 'default', transition: 'color 0.12s', '&:hover': { color: INK2 } }}>{link}</Typography>
              ))}
            </Box>
          ))}
        </Box>
