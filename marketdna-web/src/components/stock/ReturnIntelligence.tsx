...
function HistogramChart({ title, bins, stats, color }: {
  title: string
    ...
    <Box>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <Typography variant="subtitle2" sx={{ color: 'text.secondary', mb: 1, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            Daily Returns Distribution
          </Typography>
          <HistogramChart title="Daily Return %" bins={data.daily_histogram} stats={data.daily_stats} color="#4f9cf9" />
        </Grid>
        <Grid item xs={12} md={4}>
          <Typography variant="subtitle2" sx={{ color: 'text.secondary', mb: 1, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            Monthly Returns Distribution
          </Typography>
          <HistogramChart title="Monthly Return %" bins={data.monthly_histogram} stats={data.monthly_stats} color="#22c55e" />
        </Grid>
        <Grid item xs={12} md={4}>
          <Typography variant="subtitle2" sx={{ color: 'text.secondary', mb: 1, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            Yearly Returns
...
function YearlyBarChart({ data }: { data: ReturnsResponse }) {
  const options = useMemo((): Highcharts.Options => ({
    ...hcTheme,
    chart: { ...hcTheme.chart, height: 220 },
    title: { text: '' },
