...
function deriveEvents(
  ...
  if (summary.regime === 'Bullish') {
    events.push({
      ...
      body: 'Trading above SMA20, SMA50 and SMA200 simultaneously',
      type: 'positive',
    })
  } else if (summary.regime === 'Bearish') {
    events.push({
      title: 'Full Bear Structure',
      body: 'Trading below all key moving averages',
      type: 'negative',
    })
  }
  ...
  if (rs) {
    if (rs.stats.current_rank <= 5) {
      events.push({
        title: `Elite rank #${rs.stats.current_rank} of ${rs.stats.total_symbols}`,
        body: `Top ${(100 - rs.stats.rank_percentile).toFixed(0)}% by 1-month return`,
        type: 'positive',
      })
    } else if (rs.stats.current_rank >= rs.stats.total_symbols - 4) {
      events.push({
        title: `Weak rank #${rs.stats.current_rank} of ${rs.stats.total_symbols}`,
        body: 'Bottom 10% by 1-month return among NIFTY 50',
        type: 'negative',
      })
    }
  }
  ...
  if (risk) {
    if (risk.stats.atr_percentile > 85) {
      events.push({
        title: `Elevated volatility — ${risk.stats.atr_percentile}th percentile`,
        body: `ATR ₹${risk.stats.current_atr} — unusually wide daily ranges`,
        type: 'warning',
      })
    } else if (risk.stats.atr_percentile < 10) {
      events.push({
        title: `Compressed volatility — ${risk.stats.atr_percentile}th percentile`,
        body: 'ATR historically low — potential breakout setup',
        type: 'neutral',
      })
    }
  }
  ...
  if (drawdown) {
    if (drawdown.stats.current_drawdown < -20) {
      events.push({
        title: `Deep drawdown: ${drawdown.stats.current_drawdown.toFixed(1)}% from peak`,
        body: `Avg recovery takes ${drawdown.stats.avg_recovery_days} days from trough`,
        type: 'negative',
      })
    } else if (Math.abs(drawdown.stats.current_drawdown) < 1) {
      events.push({
        title: 'Near all-time high',
        body: 'Less than 1% from historical peak — strong uptrend continuation',
        type: 'positive',
      })
    }
  }
  ...
  if (percentiles) {
    for (const m of percentiles.metrics) {
      if (m.metric === '1-Month Return' && m.percentile > 92) {
        events.push({
          title: `1-Month return at ${m.percentile.toFixed(0)}th percentile`,
          body: `+${m.current_value.toFixed(1)}% — exceptional 20-day momentum`,
          type: 'positive',
        })
      }
      if (m.metric === 'Volume' && m.percentile > 90) {
        events.push({
          title: `Volume surge — ${m.percentile.toFixed(0)}th percentile`,
          body: `${m.current_value.toFixed(1)}M shares — unusually high participation`,
          type: 'neutral',
        })
      }
    }
...
export default function WhatChangedToday({ summary, rs, risk, drawdown, percentiles }: Props) {
  ...
  if (events.length === 0) {
    events.push({
      title: 'No significant events today',
      body: 'All metrics within normal historical ranges',
      type: 'neutral',
    })
  }
  ...
  return (
    <Box>
      <Box mb={2.5}>
        <Typography sx={{ fontSize: '0.75rem', fontWeight: 800, letterSpacing: '0.18em', color: GOLD, textTransform: 'uppercase', mb: 0.5, display: 'block', ...JAKARTA }}>
          Live Event Scanner
        </Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Typography sx={{ fontSize: '1.4rem', fontWeight: 800, letterSpacing: '-0.025em', color: INK, ...JAKARTA }}>
            What Changed Today
          </Typography>
          <Box sx={{ px: 1.25, py: 0.3, borderRadius: '8px', bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
            <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: INK2, ...JAKARTA }}>{events.length} event{events.length !== 1 ? 's' : ''}</Typography>
          </Box>
        </Box>
        <Typography sx={{ fontSize: '0.73rem', color: INK2, mt: 0.5, lineHeight: 1.5, ...JAKARTA }}>
        ...
        {events.map((ev, i) => {
          ...
          return (
            ...
            >
              ...
              <Box>
                <Typography sx={{ fontWeight: 700, color, fontSize: '0.85rem', lineHeight: 1.3, ...JAKARTA }}>
                  {ev.title}
                </Typography>
                <Typography sx={{ color: INK2, fontSize: '0.75rem', display: 'block', mt: 0.25, ...JAKARTA }}>
                  {ev.body}
                </Typography>
              </Box>
            </Box>
