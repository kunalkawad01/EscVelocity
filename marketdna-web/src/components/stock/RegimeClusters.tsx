...
const COND = { fontFamily: "'IBM Plex Sans Condensed'" } as const
...
export default function RegimeClusters({ data, loading }: Props) {
  ...
  const clusterColor = (label: string) => {
        ...
        <Grid item xs={12} md={4}>
          <MetricCard title={`K-Means (${kmeans.n_clusters} clusters)`}>
            ...
            <Box>
              ...
              <Typography sx={{ ...MONO, fontSize: '0.9rem', fontWeight: 800, color: retColor(kmeans.avg_fwd_return) }}>
        ...
        <Grid item xs={12} md={4}>
          <MetricCard title="Hidden Markov Model">
            <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1, mb: 1 }}>
              <Typography variant="h5" sx={{ fontWeight: 800, color: hColor }}>{hmm.state_label}</Typography>
              <Typography variant="caption" sx={{ color: INK3, fontSize: '0.75rem' }}>State</Typography>
            </Box>
            <Box sx={{ mb: 1.5 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
